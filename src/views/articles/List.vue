<template>
    <div class="col-md-9 left-col pull-right">
        <div class="panel article-body article-index">
            <div class="panel-body">
                <h1 class="all-articles">
                    专栏文章
                    <router-link v-if="auth" to="/articles/create" class="btn btn-primary pull-right">
                        <i class="fa fa-paint-brush"></i>
                        创作文章
                    </router-link>
                </h1>

                <ul class="list-group">
                    <!-- 使用 v-for 指令渲染文章列表 -->
                    <li v-for="article in articles" class="list-group-item">
                        <img :src="article.user.avatar" class="avatar avatar-small">
                        <router-link :to="`/articles/${article.id}/content`" class="title">
                            {{ article.title }}
                        </router-link>
                        <span class="meta pull-right">
              <span class="timeago">{{ article.updated_at | moment('from') }}</span>
            </span>
                    </li>
                </ul>
            </div>
        </div>
    </div>
</template>

<script>
    import {mapState} from 'vuex'
    import {getUserTopics} from '@/api/topic.js'

    export default {
        name: 'List',
        data() {
            return {
                articles: [] // 对应用户文章
            }
        },
        computed: {
            ...mapState([
                'auth',
                'user'
            ])
        },
        beforeRouteEnter(to, from, next) {
            next(vm => {
                let params = {
                    include: 'user,category'
                }
                // 确认渲染该组件的对应路由时，获取对应用户文章
                getUserTopics(to.params.userId, params).then((res) => {
                    vm.articles = res.data.data
                })
                // vm.articles = vm.$store.getters.getArticlesByUid(null, to.params.user)
            })
        }
    }
</script>

<style scoped>

</style>
